...
class TimestampTest : public pg::test::TypeTest {
    ...
    auto read(std::string_view string) -> timestamp {
        ...
        run([&]() -> ext::task<> {
            result = co_await client->fetch<timestamp>(
                fmt::format("SELECT '{}'::timestamptz", string)
            );
        }());
    ...
    auto write(timestamp ts) -> std::string {
        ...
        run([&]() -> ext::task<> {
            ...
            result = co_await client->query("SELECT $1::timestamptz", ts);
        }());
...
TEST_F(TimestampTest, Read) { read_time("2010-05-25 17:30:26"); }
