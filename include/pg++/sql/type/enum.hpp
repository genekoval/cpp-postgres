    ...
    struct pg::type<Type> {                                                    \
        ...
        static auto from_sql(                                                  \
            ...
            netcore::buffered_socket& reader                                   \
                                                                               ...
                                                                               \
        static auto to_sql(Type t, netcore::buffered_socket& writer)           \
            -> ext::task<>;                                                    \
                                                                               \
...
#define PG_ENUM_DEFINE(Type)                                                   \
    auto pg::type<Type>::from_sql(                                             \
        ...
        ->ext::task<Type> {                                                    \
        const auto string =                                                    \
            co_await pg::type<std::string>::from_sql(size, reader);            \
        co_return pg::enum_type<Type>::from_string(string);                    \
    }                                                                          \
                                                                               \
    auto pg::type<Type>::to_sql(Type t, netcore::buffered_socket& writer)      \
        ->ext::task<> {                                                        \
...
namespace pg {
    ...
    template <typename T>
    concept sql_enum =
        std::is_enum_v<T> && requires(T t, std::string_view string) {
            {
